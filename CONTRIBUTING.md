...
| Code Executor (default) | GLM 5.1 | opencode + OmniRoute | `src/`, `tests/`, append-only to `docs/tickets/<id>.md#10 Execution Log`, create files in `docs/questions/` | `docs/prd/`, `docs/architecture/`, anything outside the assigned ticket's §5 Outputs |
...
6. **Executor guardrails.**
   - Executor may modify ONLY files explicitly listed in the Ticket's §5 Outputs.
