...
**Update 2026-05-02 (pt2 — research note opened in PR #93):** `docs/knowledge/llm-model-evaluation-2026-05.md` compiles BenchLM provisional / lmarena verified / Fireworks operational specs for the six candidate models (GPT-5.5 family + GLM 5.1 + DeepSeek V4 Pro + Qwen 3.6 Plus + Kimi K2.6 + MiniMax M2.7) and proposes a role→model matrix for PO review. **PR-Agent specific recommendation:** swap from Qwen 3.6 Plus to **MiniMax M2.7** (cheapest output $1.20/1M tokens; marketing matches PR-Agent agent-harness profile; uncorrelated from primary-Reviewer Kimi K2.6 + Executor families GLM/DeepSeek/Codex), with rollback trigger = 2-of-3 stalls in first 3 piloted PRs (fall back to GLM 5.1 temporarily, then re-pilot DeepSeek V4 Pro). See research note §4 for full matrix and §5 for implementation plan including telemetry capture protocol that further narrows this investigation track.
