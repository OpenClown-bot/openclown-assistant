...
## 5. Architect-4 synthesis conclusion (PR-D)

Final canonical choice for ARCH-001@0.5.0 is HYBRID OpenClaw Gateway + KBJU Node 24 sidecar via HTTP bridge. PR-C remains the empirical base for the alternatives matrix, but Architect-4 patches two details: the stall-watchdog default threshold is 120 seconds per PRD-002@0.2.1 G2 (with ≤15 seconds post-threshold event emission), and generic skill registry / mesh delegation is deferred because PRD-002@0.2.1 does not require it.

Load-bearing imports from this comparison:
- OpenClaw stays because it is the only language/runtime match and PO-constrained incumbent.
- zeroclaw contributes the polling-pattern inspiration for C13, not Rust code or a transport-level watchdog.
- hermes-agent and nanobot remain reference patterns only; no cross-runtime mesh is accepted for v0.5.0.
- ironclaw registry remains future reference; no TKT is created for it in PR-D.
