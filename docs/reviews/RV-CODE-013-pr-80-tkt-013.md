...
## Summary
PR #80 delivers TKT-013@0.1.0 Deployment Packaging with 35/35 passing tests, clean lint/typecheck, and valid Docker Compose configuration. Executor iter-5 (commits `69ead5d` and `b50443e`) resolved the cross-reviewer metrics-bind concern by changing `METRICS_HOST` from `"0.0.0.0"` to `"metrics"` (Docker-internal hostname), updating the container healthcheck URL to `http://metrics:9464/healthz`, adding `healthCheck.test.ts` tests that assert wildcard rejection, and adding `compose.test.ts` assertions that `METRICS_HOST` is loopback or Docker-internal. Two low-severity test-quality follow-ups remain (F-L3, F-L4), neither blocking merge. One new low finding from PR-Agent review (missing HTTP server `error` handler) is also non-blocking.
...
One-sentence justification: All high and medium findings from prior review iterations are resolved. Three low-severity items remain: cleaner JSON parse error messages (F-L3), rollback success-path test coverage (F-L4), and missing HTTP server `error` handler (F-L6); all are non-blocking follow-ups.
...
| 4 | `b50443e5097b9cfa3b54ec98f4220ea46f94f1f9` | pass_with_changes | Cross-reviewer metrics-bind concern resolved: `METRICS_HOST` changed to Docker-internal hostname `"metrics"`; healthcheck URL updated; `healthCheck.test.ts` added with wildcard-rejection tests. F-L3, F-L4, and new F-L6 (missing server error handler) remain as low follow-ups. |
...
- [x] CI green (lint, typecheck, tests, coverage)
  - `npm test` 35/35 pass; `npm run lint` / `npm run typecheck` exit 0; `docker compose config` exits 0.
...
| F-M3 | Dockerfile `HEALTHCHECK` invalid for `app` service + metrics wildcard bind | Medium | **RESOLVED** — Image-level `HEALTHCHECK` removed from `Dockerfile`; service-level `healthcheck` added to `metrics` in `docker-compose.yml`; `METRICS_HOST` changed from `"0.0.0.0"` to `"metrics"` (Docker-internal hostname); healthcheck URL updated to `http://metrics:9464/healthz`; `healthCheck.test.ts` (5 tests) added asserting wildcard rejection; `compose.test.ts` asserts `METRICS_HOST` is loopback or Docker-internal hostname. |
...
- **F-L6 (`src/deployment/healthCheck.ts:20–52`):** The `http.createServer()` instance does not register an `'error'` event listener on the server object itself. If `server.listen()` fails (e.g., port 9464 already in use, or the `metrics` hostname fails to resolve at startup), the resulting `Error` event on the `server` object is unhandled and will crash the Node process. In the dedicated `metrics` Docker service this is an unlikely race condition, but an unhandled error event is a reliability gap. — *Responsible role:* Executor (deferred). *Suggested remediation:* Add `server.on('error', (err) => { console.error('Metrics server error:', err.message); process.exit(1); });` before `server.listen()` to catch bind failures gracefully.
...
| 4 | Metrics `0.0.0.0` container-internal binding + host `127.0.0.1` port mapping | **F-M3 — RESOLVED in iter-5** | `METRICS_HOST` changed to `"metrics"` (Docker-internal hostname); `ports: 127.0.0.1:9464:9464` remains for host-only access. |
...
| 6 | Missing HTTP server `error` handler in `healthCheck.ts` | **F-L6 (Low)** | Unhandled `'error'` event on `server` object if `listen()` fails; low likelihood in dedicated container but a reliability gap. |

## AC-by-AC verification summary (iter-4)

| # | AC text | Status | Evidence / Notes |
|---|---------|--------|------------------|
| 1 | `npm test -- tests/deployment/compose.test.ts tests/deployment/envExample.test.ts` passes | **PASS** | 35/35 tests pass across 4 test files (`compose.test.ts`: 16 tests, `envExample.test.ts`: 5 tests, `scripts.test.ts`: 9 tests, `healthCheck.test.ts`: 5 tests). |
...
Iter-4: `b50443e5097b9cfa3b54ec98f4220ea46f94f1f9`

### C. Commands run (iter-4)
```bash
npm test -- tests/deployment              # 35/35 pass (4 files)
npm run lint                             # exit 0
