...
- **PR-Agent state on final Executor HEAD**: workflow run id + `conclusion: success` + persistent-review `updated_until_commit` matches final Executor HEAD + classified findings list. If the PR-Agent state is `IN_PROGRESS`, `failure`, `cancelled`, or stale on a prior HEAD → the hand-back is incomplete; bounce. The TO is required to wait for PR-Agent settlement before handing back (see `docs/prompts/ticket-orchestrator.md` § PR-Agent settle-on-final-HEAD requirement). **Exception: PR-Agent CI workflow tail-latency stall** is a structural pattern (5-of-5 final-HEAD code-PRs across all 5 TO pilots TKT-010/011/013/012/014 stalled or cancelled at the PR #71 12-min hard-timeout — see `docs/backlog/deployment-followups.md` §TKT-NEW-pr-agent-ci-tail-latency-investigation-CRITICAL); when the persistent review settles to the final HEAD with clean verdict but the workflow itself is stuck or cancelled, treat as expected infra failure under DO authority and proceed. The TO must capture the stall timing in §TO Operational Notes (see below).
- Cross-reviewer audit pass-1 list — every Reviewer finding AND every PR-Agent finding (inline `/improve` + persistent-review) by id with classification (RESOLVED / promoted-to-iter-N+1 / deferred-to-BACKLOG-X TKT-NEW-Y) and one-line rationale. Mark which findings overlap between Kimi and PR-Agent.
- Strategic blockers (none, or list).
- Pending closure-PR scope.
- **TO Operational Notes (for Devin Orchestrator postmortem capture).** Mandatory section added 2026-05-02 in closure-PR #91 per `BACKLOG-011 §TKT-NEW-to-do-postmortem-loop-formal-channel`. **Purpose:** capture procedural insights observed during the cycle that are NOT Reviewer findings on the code but are pipeline / process observations. **Structure:** free-form bullet list. Each bullet should describe ONE observation in 1-3 sentences with: (a) what happened, (b) why it happened (best understanding), (c) the impact on the cycle, (d) whether the TO worked around it or was blocked. **Examples of what belongs here:** Executor model failure modes (context exhaustion, throughput issues, push auth fails, takeover clobbers); opencode session interruptions; PR-Agent CI behavior anomalies; cross-reviewer audit difficulties; unexpected ArchSpec / TKT contradictions discovered mid-cycle; PO-mediated decisions and their impact. **Examples of what does NOT belong here:** Reviewer code findings (those go in `RV-CODE-NNN` artifact + per-finding resolution table); deferred follow-ups (those go in BACKLOG via closure-PR); routine cycle metrics (those go in TKT §10 Execution Log). **Integration with pass-2:** the Devin Orchestrator pass-2 ratification (§11.4) MUST read the TO Operational Notes and decide for each bullet: (a) codify as a new BACKLOG TKT-NEW entry (structural / portable lesson), (b) note in session-log only (single-pilot anomaly, archive), or (c) escalate to PO via chat (urgent / requires PO input). The decision is recorded in the closure-PR body and/or session-log. If the TO Operational Notes section is missing entirely, that is a hand-back-format violation; bounce. If the section is present but empty, the TO must affirmatively state "no operational anomalies this cycle" rather than omit the section. The first 5 TO pilots produced operational notes in retrospect through PO chat (e.g. Qwen 128k context fail on TKT-014, opencode-session interrupts on TKT-013, F-PA-17 miss origin on TKT-010); these patterns are codified in `BACKLOG-007/008/009/010/011`.
