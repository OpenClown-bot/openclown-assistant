---
id: ADR-011
title: "Runtime architecture choice — 5 alternatives evaluated"
version: 0.1.0
status: proposed
...
**Chosen: HYBRID (Option E) — OpenClaw Gateway + KBJU sidecar Node 24 process bridged via HTTP.**
...
cost on existing 15 merged tickets — the sidecar reuses `src/` modules directly with a new `src/main.ts`
HTTP entrypoint.
